...
def imprimir(matriz):
    ...
    while i < len(matriz):
        ...
        print()
        i += 1
    return vive_muere(matriz,len(matriz),len(matriz[0]),2,3,6)
...
def ver_vecinos(tablero, y,x,largo,ancho,mini,maxi,nace):
 ...
 #Entra primero a ver si esquina, luego bordes, luego normal
    ...
    if y == 0: 
    #Para las celulas de la primera fila, ya que son borde
        vecinas = [tablero[y+1][x],tablero[y][x-1],tablero[y][x+1],tablero[y+1][x-1],tablero[y+1][x+1],tablero[largo-1][x-1],tablero[largo-1][x],tablero[largo-1][x+1]]
        for vecina in vecinas:
            if vecina == "\u25A0":
                celulas+=1
    elif x == 0: #Para el borde de la primera columna, donde x es siempre 0
        vecinas = [tablero[y][x-1], tablero[y+1][x],tablero[y-1][x],tablero[y-1][x+1],tablero[y+1][x+1],tablero[y-1][largo-1],tablero[y][ancho-1],tablero[y+1][ancho-1]]
        for vecina in vecinas:
            if vecina == "\u25A0":
                celulas +=1
