...
def vive_muere(tablero,y,x,nace1,nace2,mini,maxi):
    ...
    while i < len(tablero):
        ...
        while j < len(tablero[0]):
                ...
                vive = ver_vecinos(tablero,i,j,y,x,nace1,nace2,mini,maxi)
                if vive == False:
                    tablero[i][j] = " "
                else:
                    tablero[i][j] = "\u25A0"
...
def ver_vecinos(tablero, y,x,largo,ancho,nace1,nace2,mini,maxi):
 ...
 #Entra primero a ver si esquina, luego bordes, luego normal
    ...
    if celula == "\u25A0":
        if celulas in range(mini,maxi+1):
            ...
            return False
    if celula == " ":
        if celulas == nace1 or celulas == nace2:
            return True
        else:
            return False
